...
_v_var(coupledValue("v")),// _v_var WILL HOLD TEMPERATURE VALUE
  ...
  _Ttrig(getParam<Real>("Ttrig")),_Tintl(getParam<Real>("Tintl")),E(getParam<Real>("_E")),n(getParam<Real>("_n")),m(getParam<Real>("_m"))
...
Real falpha = pow(1.0-_u[_qp],n)*pow(_u[_qp],m)*(1/(exp(14.48*(_u[_qp]-0.77))));
    /* To be copied to QpJacobian */
...
//Cure dependence: f(alpha)
Real falpha = pow(1.0-_u[_qp],n)*pow(_u[_qp],m)*(1/(exp(13.5*(_u[_qp]-0.55))));
    /* To be copied to QpJacobian */
Real dfalpha = - n*pow(1-_u[_qp],n-1)*pow(_u[_qp],m)*(1/(exp(14.48*(_u[_qp]-0.77))))+m*pow(_u[_qp],m-1)*pow(1.0-_u[_qp],n)*(1/(exp(14.48*(_u[_qp]-0.77))))+pow(1.0-_u[_qp],n)*pow(_u[_qp],m)*((-14.48*(exp(14.48*(_u[_qp]-0.77))))/(pow(exp(14.48*(_u[_qp]-0.77)),2)));
//Cure function is product of K(T) and falpha
Real Pc = KT*falpha;
